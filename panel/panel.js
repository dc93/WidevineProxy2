...
async function createCommand(json, key_string, videoName) {
    ...
    return `${await SettingsManager.getExecutableName()} "${metadata.url}" ${header_string} ${key_string} ${await SettingsManager.getUseShakaPackager() ? "--use-shaka-packager " : ""}--auto-select --save-name "${safeVideoName}" -M format=mkv`;
}
